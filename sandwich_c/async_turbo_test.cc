...
///
/// \file
/// \brief Unit tests for OpenSSL turbo-tunnels using the C frontend.
...
/// \brief Create a Turbo Listener using sandwich API.
///
/// \return A Sandwich Turbo Listener for the server.
auto CreateTurboListener(std::string udp_ipaddr, uint16_t udp_port, std::string tcp_ipaddr, uint16_t tcp_port, bool is_blocking)
    -> std::unique_ptr<struct ::SandwichListener, SandwichListenerDeleter> {
  saq::sandwich::proto::api::v1::ListenerConfiguration config{};

  config.mutable_turbo()->mutable_udp()->set_hostname(udp_ipaddr);
  config.mutable_turbo()->mutable_udp()->set_port((uint32_t)udp_port);
  config.mutable_turbo()->mutable_tcp()->set_hostname(tcp_ipaddr);
  config.mutable_turbo()->mutable_tcp()->set_port((uint32_t)tcp_port);
  sandwich_assert(is_blocking == false);
  config.mutable_turbo()->set_blocking_mode(saq::sandwich::proto::api::v1::BlockingMode::BLOCKINGMODE_NONBLOCKING);
  std::string encoded_configuration{};
  sandwich_assert(config.SerializeToString(&encoded_configuration) == true);
...
if (err != NULL) {
	std::cout << sandwich_error_stack_str_new(err) << std::endl;
}
...
void ServerAnswerHandshake(struct ::SandwichTunnel *server) {
  ...
  enum ::SandwichTunnelHandshakeState state;
  struct SandwichError *err = NULL;
  // Because of request-based fragmentation we may need to try a few times.
  for (int i = 0; i < 10; i++) {
	err = ::sandwich_tunnel_handshake(server, &state);
  }
  sandwich_assert(err == NULL);
  sandwich_assert(state == SANDWICH_TUNNEL_HANDSHAKESTATE_WANT_READ);
...
void ClientCompleteHandshake(struct ::SandwichTunnel *client) {
  ...
  } while (state != SANDWICH_TUNNEL_HANDSHAKESTATE_DONE);
  sandwich_assert(err == NULL);
...
void ClientReadPong(struct ::SandwichTunnel *client) {
  MsgBuffer buffer{};
  size_t r{0};
  enum SandwichTunnelRecordError err;
  do {
	  err = ::sandwich_tunnel_read(client, buffer.data(), buffer.size(), &r);
  } while (err == SANDWICH_TUNNEL_RECORDERROR_WANT_READ);
  sandwich_assert(err == SANDWICH_TUNNEL_RECORDERROR_OK);
  sandwich_assert(r == kPongMsg.size());
...
int main(int argc, char **argv) {
  ...
  auto server_listener = CreateTurboListener(server_hostname.c_str(),
		  				server_port,
  ...
  // Unlike in tunnels_test.cc, we have to create the client tunnel before we
  // call accept on a turbo listener. This is because the Turbo listener depends
  // on the UDP handshake packets to count as "connected". These are only sent
  // once a handshake has started.

  // Create tunnels.
  auto client_tunnel_io = ::sandwich_owned_io_to_tunnel_io(client_io);

  auto client_tunnel = CreateTunnel(&*client, client_tunnel_io,
                                    SandwichTunnelConfigurationVerifierEmpty);
  // Client initiates the handshake.
  ClientInitiateHandshake(&*client_tunnel);

  do {
  	err = sandwich_listener_accept(&*server_listener, &server_io);
  } while (err != SANDWICH_IOERROR_OK);

  auto server_tunnel_io = ::sandwich_owned_io_to_tunnel_io(server_io);

  auto server_tunnel = CreateTunnel(&*server, server_tunnel_io,
                                    SandwichTunnelConfigurationVerifierEmpty);
