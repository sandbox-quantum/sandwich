...
/// Targets to build.
const TARGETS: [&str; 3] = [
    "//rust:boringssl_bindgen",
    "//rust:boringssl_bartleby",
    "//rust:boringssl_bindgen__bindgen",
];

/// Foreign C library to link Sandwich against, in Bazel.
const BAZEL_TARGET_LIBRARY: &str = "libboringssl_bartleby_bartleby.a";

/// New name of the foreign C library to link Sandwich against.
const OUT_LIBRARY: &str = "libsandwichboringssl.a";

/// Name of the bindgen file produced by Bazel.
const BAZEL_TARGET_BINDGEN: &str = "boringssl_bindgen__bindgen.rs";
...
fn main() {
    ...
    let library = cargo_unwrap!(
        ...
        "cannot construct library output path"
    );

    let bindgen_rs = cargo_unwrap!(
        build_support::out_dir_path("lib.rs"),
        "cannot construct `lib.rs` path"
    );
    ...
    cargo_unwrap!(
        ...
        "failed to build BoringSSL and its bindgen"
    );
