...
///         X509Identity: `dilithium5.cert.pem` and `dilithium5.key.pem`
///         alpn_protocols: none.
...
///         kem: kyber768
///         X509Verifier:
///             trusted_cas: `dilithium5.cert.pem`
///             allow_expired_certificate: false
///             max_verify_depth: default (100)
...
fn test_ok() {
    ...
    let (client_cert_path, client_key_path) = (
        runfile("testdata/dilithium5.cert.pem"),
        runfile("testdata/dilithium5.key.pem"),
    );
    let (server_cert_path, server_key_path) = (
        runfile("testdata/falcon1024.cert.pem"),
        runfile("testdata/falcon1024.key.pem"),
    );
...
/// Test a mutual TLS tunnel. Same test as above, but the client is not
...
///         TunnelVerifier: empty.
#[test]
fn test_nok_client_lack_of_identity() {
    let lib_ctx = sandwich::Context::new();

    let client_cert_path = runfile("testdata/dilithium5.cert.pem");
    let (server_cert_path, server_key_path) = (
        runfile("testdata/falcon1024.cert.pem"),
        runfile("testdata/falcon1024.key.pem"),
    );
    ...
    server_tunnel.handshake().unwrap_err();
}
